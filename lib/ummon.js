  ...
  self.on('task.*', function(taskid){
    var wildCardTriggerTasks = _.filter(self.tasks, function(task){
      ...
      return (
        ...
        || ('string' === typeof task.trigger && ~task.trigger.indexOf('*'))
      )
    })

    if (wildCardTriggerTasks.length) {
      wildCardTriggerTasks.forEach(function(task){
    ...
    if (completedRun.task.id) {
      ...
      if (refs.length) {
        for (var index = 0; index < refs.length; index++) {
          self.log.debug({event: 'worker.complete'}, '%s triggered %s', completedRun.task.id, refs[index]);
...
Ummon.prototype.getTaskId = function(name){
  ...
  return (self.doesTaskExist(name)) ? name : false;
...
Ummon.prototype.setupTaskdependencies = function(t){
  ...
  if (trigger !== '*' && trigger.indexOf('.') === -1) {
    trigger = t.collection + '.' + trigger;
  }

  tasks = self.getTaskIds(trigger);
  // If there is a wildcard for the tasks own collection,
...
Ummon.prototype.updateTask = function(taskid, options, callback){
  ...
  self.removeTaskdependencies(task.id);
  self.setupTaskdependencies(task);
