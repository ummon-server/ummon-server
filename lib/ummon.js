...
Ummon.prototype.setupTaskDependancies = function(t){
  ...
  if (self.tasks[t.id].trigger.before) {
    var before = self.taskName(self.tasks[t.id].trigger.before);

    if (!self.doesTaskExist(before)) {
      self.log.error('Task trigger ' + before + ' does not exist');
    }
    self.dependencies.subject(before).dependOn(t.id);
  }
