...
Ummon.prototype.createTask = function(config){
  ...
  var t = new task(config);

  if (Object.keys(self.tasks).indexOf(t.id) !== -1) {
    self.log.error(new Error('A task with name "' + t.id + '"" already exists'));
  }
...
Ummon.prototype.setupTaskDependancies = function(t){
  ...
  if (self.tasks[t.id].trigger.after) {
    self.dependencies.subject(t.id).dependOn(self.tasks[t.id].trigger.after);
  }

  if (self.tasks[t.id].trigger.before) {
    self.dependencies.subject(self.tasks[t.id].trigger.before).dependOn(t.id);
  }
 ...
 * Run a task!
 */
Ummon.prototype.run = function(t){
  var self = this;

  var workerProcess = worker(t, self);
  ...
  self.dispatcher.once('worker.complete', function(exitCode){
    ...
    var refs = self.dependencies.subject(t.id).references;
