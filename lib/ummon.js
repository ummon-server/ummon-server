...
Ummon.prototype.getTasks = function(collection, callback){
  ...
  var collectionNames = (collection) 
    ...
    : _.uniq(_.pluck(self.tasks, 'collection'), true);

  // Filter the tasks based on the collections we want
  for (var tid in self.tasks){
    if (~collectionNames.indexOf(self.tasks[tid].collection)) {
      tasks.push(tid);
    }
  }

  // Load the tasks
  async.map(tasks, self.getTask.bind(self), function(err, tasks){
    collectionNames.forEach(function(collection){
      collections.collections[collection] = {
        defaults: self.defaults[collection],
        tasks: _.filter(tasks, function(task){ return task.collection === collection; })
      };
    });
