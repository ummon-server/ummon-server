...
// Inherit the EventEmitter2 constructuor
...
function Ummon(options) {
  ...
  d.run(function () {
    ...
    }, self.config.createWorkerPollInterval);
...
Ummon.prototype.setupListeners = function (taskid) {
  ...
  self.on('task.*', function (taskid) {
    var wildCardTriggerTasks = _.filter(self.tasks, function (task) {
      // Skip tasks without triggers or the task that just changed
      if (!('trigger' in task) || task.id === taskid) {
        return false;
      }
    ...
    if (completedRun.task.id) {
      ...
      if (task) {
        ...
        if (status === 'success') {
          ...
          task.totalSuccessfulRuns++;
        } else {
          task.totalFailedRuns++;
        }
      }
 ...
 *
 * @param {String} selector Optional wildcard selector, eg: "ummon.*"
 * @return {Array} All collection names
 */
Ummon.prototype.getTaskIds = function (selector) {
  ...
  if (arguments.length == 0 || selector == '*') {
    // this function was called without args to get all task IDs, or we're explicitly
    // looking for all task IDs
    return taskids;
  } else if (selector === false) {
    // this function was called with a selector that isn't a valid task
    return [];
  } else {

    // Assume the selector is "collectionName.*" and remove the star
...
Ummon.prototype.setupTaskTriggers = function (task) {
  ...
  if (task.trigger) {
    ...
    if ('string' === typeof task.trigger) {
      task.trigger = isCronTime(task.trigger) ? {time: task.trigger} : {after: task.trigger};
    }

    ['after', 'afterFailed'].forEach(function (key) {
...
Ummon.prototype.getTaskDependencies = function (taskid, status) {
  ...
  if (taskid in self.tasks
    ...
    taskDependencies = triggerTaskIds.filter(function (triggerTaskId) {
      // Don't let a task depend upon itself
      return triggerTaskId !== taskid;
    });
  }
...
Ummon.prototype.createTimer = function (t) {
  ...
  if (t.trigger.time && self.isTaskEnabled(t)) {
    try {
      new cronJob(t.trigger.time, function () {
 ...
 *
 * @param  {string} taskid
 * @param  {Task} options   Config data
 * @param {Function}    callback
 */
...
Ummon.prototype.runTask = function (task, callback) {
  var self = this;
  var taskToRun, tid = self.getTaskId(task);
