 ...
 * @return {string}     
 */
Ummon.prototype.taskId = function(name){
...
Ummon.prototype.setupTaskdependencies = function(t){
  ...
  if (t.trigger.after || ('string' === typeof t.trigger && !isCronTime(t.trigger))) {
    var after = self.taskId(t.trigger.after || t.trigger);

    if (!self.doesTaskExist(after)) {
      self.log.error('Task trigger ' + after + ' does not exist');
    }
    self.dependencies['success'].subject(t.id).dependOn(after);
  }

  if (t.trigger.afterFailed) {
    var afterFailed = self.taskId(t.trigger.afterFailed);

    if (!self.doesTaskExist(afterFailed)) {
      self.log.error('Task afterFailed trigger ' + afterFailed + ' does not exist');
    }
    self.dependencies['error'].subject(t.id).dependOn(afterFailed);
  } 
};
...
function isCronTime(cronString) {
  return (cronString[0] === '*' || !isNaN(cronString[0]));
}