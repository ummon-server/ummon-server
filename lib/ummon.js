      ...
      if (completedRun.task.id) {
        ...
        if (refs.length && completedRun.exitCode === 0) {
          for (var index = 0; index < refs.length; index++) {
            ...
            self.queue.push(self.getTask(refs[index]), completedRun.id);
          }
        }
      }
    });
...
Ummon.prototype.autoLoadTasks = function(callback){
  ...
  glob(self.config.tasksDir, function (err, files) {
    
    if (err || !files.length) {
      self.log.error(err | "No task config files were found");
      return;
    }
    ...
    for (var index = 0; index < files.length; index++) {
      var config;
      try {
        config = require(path.resolve(files[index]));
      } catch(e) {
        self.log.error("Failed to load task file (%s): %s", files[index], e.message);
      }

      for (var collection in config) {
        self.createCollectionAndTasks(collection, config[collection]);
      }
    }
...
Ummon.prototype.createCollectionAndTasks = function(collection, config){
  var self = this;

  // Setup the defaults for this collection
  self.defaults[collection] = config.defaults;

  // Create each task
  for (var task in config.tasks){
    self.createTask(config.tasks[task], task, collection);
  }
  ...
  if (config.sequences) {
    config.sequences.forEach(function(sequence){
      ...
      if (dependent.indexOf('.')) { dependent = collection + '.' + dependent; }
 ...
 *
 * TODO: Make this async so a smarter callback(err) approach can be used
 *
 * Example:
 *
 ...
 */
Ummon.prototype.createTask = function(config, name, collection){
  var self = this;
  ...
  if (!config.name){
    if (name) {
      ...
      return self.log.error(new Error('No task name provided for task: ' + config.command));
    }
  }

  if (!config.collection) { 
    config.collection = (collection) ? collection : self.config.defaultCollection;
  }
...
Ummon.prototype.getTasks = function(){
  var self = this;
  var tasks = [];

  for (var tid in self.tasks){
    tasks.push(self.getTask(tid));
  }

  return _.groupBy(tasks, 'collection');
}
