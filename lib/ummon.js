...
Ummon.prototype.setupListeners = function(taskid) {
  ...
  self.on('queue.new', function(r){
    self.log.info({event: 'queue.new', runid:r.id, taskid: r.task.id, collection: r.task.collection}, "queue.new - %s added . Queue length: %d", r.task.id, self.queue.length());
...
Ummon.prototype.getTaskId = function (name, task) {
  ...
  if ('string' === typeof name && name.indexOf('.') === -1 && name.indexOf('*') === -1) {
    // If name is in this task's collection return that
    if (task && self.doesTaskExist(task.collection+'.'+name)) {
...
Ummon.prototype.getTaskDependencies = function(taskid, status) {
  ...
  if (taskid in self.tasks
    ...
    var taskIds = self.getTaskIds(self.tasks[taskid].trigger[triggerKey]);
 ...
 *
 * Task can be an object: {command:'echo "meat!"'}. 
 *
