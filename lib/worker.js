...
function Worker(run, ummon){
  ...
  self.log = ummon.log.child({runid: run.id, taskid: run.task.id, collection: run.task.collection});
  ...
  d.run(function() {
    ...
    running.on('close', function (code) {
      run.complete(code);
      self.log.info({run: run}, "worker.complete in " + run.duration()/1000 + " seconds with exit code " + code);
